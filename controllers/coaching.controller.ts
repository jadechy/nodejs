...
export class CoachingController {
  constructor(
    public readonly coachingService: CoachingService,
  ...
  async updateCoaching(req: Request, res: Response) {
    try {
      ...
      const updatedCoaching = await this.coachingService.updateCoaching(
        coachingId,
        updateData
      );
  ...
  async getCoachingSessionById(req: Request, res: Response) {
    try {
      const coachingId = req.params.id;
      const coaching = await this.coachingService.findCoachingSessionById(
        coachingId
      );
  ...
  async joinCoaching(req: Request, res: Response) {
    ...
    try {
      ...
      const coaching = await this.coachingService.joinCoaching(
        coachingId,
        userId
      );
      res.status(200).json(coaching);
