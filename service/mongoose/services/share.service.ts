...
export class ShareService{
    ...
    async createShare(share: CreateShare): Promise<Share> {
        ...
        const fromId = typeof from === 'string' ? new Types.ObjectId(from) : new Types.ObjectId(from._id);
        const toId = typeof to === 'string' ? new Types.ObjectId(to) : new Types.ObjectId(to._id);
        const challengeId = typeof challenge === 'string' ? new Types.ObjectId(challenge) : new Types.ObjectId(challenge._id);

        const existingShare = await this.shareModel.findOne({ from: fromId, to: toId, challenge: challengeId });
        if (existingShare) {
        ...
        const populated = await this.shareModel.findById(created._id)
            .populate('from')
            .populate('to')
            .populate('challenge');
    ...
    async getAllMyShare(userId: string): Promise<Share[]> {
        return this.shareModel.find({ to: userId })
            .populate('from')
            .populate('challenge');
    }
}