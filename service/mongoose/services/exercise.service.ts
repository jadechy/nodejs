...
export class ExerciseService {
  ...
  findAllExercises = async (): Promise<Exercise[]> => {
    return this.exerciseModel.find();
  };

  findExerciseById = async (id: string): Promise<Exercise | null> => {
  ...
  createExercise = async (exercise: CreateExercise): Promise<Exercise> => {
  ...
  deleteExercise = async (exeId: string): Promise<void> => {
    if (!isValidObjectId(exeId)) {
