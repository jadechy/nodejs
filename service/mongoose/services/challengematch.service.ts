...
export type CreateChallengeMatch = Omit<
  ChallengeMatch,
  "_id" | "createdAt" | "updatedAt"
>;
export type UpdateChallengeMatch = Partial<
  Omit<ChallengeMatch, "_id" | "createdAt">
>;
...
export class ChallengeMatchService {
  ...
  constructor(public readonly connection: Mongoose) {
    this.challengeMatchModel = connection.model(
      "ChallengeMatch",
  ...
  createChallengeMatch = async (
    challengeMatch: CreateChallengeMatch
  ): Promise<ChallengeMatch> => {
    const challengeId = new Types.ObjectId(
      typeof challengeMatch.challenge === "string"
        ? challengeMatch.challenge
        : challengeMatch.challenge._id
    );
    const challengerId = new Types.ObjectId(
      typeof challengeMatch.challenger === "string"
        ? challengeMatch.challenger
        : challengeMatch.challenger._id
    );
    const opponentId = new Types.ObjectId(
      typeof challengeMatch.opponent === "string"
        ? challengeMatch.opponent
        : challengeMatch.opponent._id
    );
    ...
    const existing = await this.challengeMatchModel.findOne({
      ...
      opponent: opponentId,
      status: {
        $in: [ChallengeMatchStatus.PENDING, ChallengeMatchStatus.ACCEPTED],
      },
    });
    if (existing) {
      throw new Error(
        "Un défi est déjà en cours entre ces deux utilisateurs pour ce challenge."
      );
    }
    ...
    const populated = await this.challengeMatchModel
  ...
  findAllMatchRequestsByUser = async (
    userId: string
  ): Promise<ChallengeMatch[]> => {
    return this.challengeMatchModel
  ...
  addTrainingToMatch = async (
    matchId: string,
    userId: string,
    updateData: UpdateChallengeMatch
  ): Promise<ChallengeMatch> => {
    ...
    const updatedMatch = await this.challengeMatchModel
      .findByIdAndUpdate(matchId, { $set: updateFields }, { new: true })
      .populate("challenge")
      .populate("challenger")
      .populate("opponent")
      .populate("challengerTraining")
  ...
  computeMatchWinner = async (matchId: string): Promise<ChallengeMatch> => {
    if (!isValidObjectId(matchId)) {
      throw new Error("ID de match invalide.");
    }

    const match = await this.challengeMatchModel
      .findById(matchId)
      .populate("challenger")
      .populate("opponent")
      .populate({
    ...
    if (!match.challengerTraining || !match.opponentTraining) {
      throw new Error(
        "Les deux trainings doivent être renseignés pour calculer un gagnant."
      );
    }
    ...
    if (
      winner &&
      match.challenge &&
      typeof match.challenge !== "string" &&
      match.challenge.reward
    ) {
      const rewardId =
        typeof match.challenge.reward === "string"
    ...
    const matchUpdated = await this.challengeMatchModel
      .findById(matchId)
      .populate("challenge")
      .populate("challenger")
      .populate("opponent")
  ...
  private calculatePoints = (
    ...
    const training =
      role === "challenger" ? match.challengerTraining : match.opponentTraining;
    const otherTraining =
      role === "challenger" ? match.opponentTraining : match.challengerTraining;
    ...
    const extraExercises = [...performedSet].filter(
      (ex) => !challengeSet.has(ex)
    );
    ...
    if (
      training.createdAt &&
      otherTraining.createdAt &&
      training.createdAt < otherTraining.createdAt
    ) {
      points++;
    }
